...
// Trainer menolak course request miliknya
router.patch(
  '/:id/reject',
  authMiddleware,
  roleMiddleware('TRAINER'),
  async (req, res) => {
    try {
      ...
      if (!Number.isInteger(requestId)) {
        return res.status(400).json({
          message: 'ID course request tidak valid'
        })
      }
      ...
      if (
        ...
        return res.status(403).json({
          message:
            'Course request ini bukan milik Anda'
        })
      }
      ...
      if (existingRequest.status !== 'pending') {
        return res.status(400).json({
          message:
            'Hanya request berstatus pending yang dapat ditolak'
        })
      }
        ...
        async (tx) => {
          ...
          await tx.notification.create({
            data: {
              ...
              title:
                'Course Request Ditolak Trainer',
              message:
                `Trainer menolak request untuk program: ${existingRequest.program.program_name}`
            }
          })
          ...
          await tx.auditLog.create({
            data: {
              ...
              entity_id: requestId,
              description:
                `Trainer menolak course request untuk program ${existingRequest.program.program_name}`
            }
          })
      ...
      return res.status(200).json({
        message:
          'Course request berhasil ditolak',
        request: result
      })
    } catch (err) {
      console.error(
        'REJECT COURSE REQUEST ERROR:',
        err
      )
